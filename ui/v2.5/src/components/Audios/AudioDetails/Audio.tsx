...
import {
  getAbLoopPlugin,
  getPlayerPosition,
} from "src/components/AudioPlayer/util";
import {
  faEllipsisV,
  faChevronRight,
  faChevronLeft,
...
const AudioVideoFilterPanel = lazyComponent(
  () => import("./AudioVideoFilterPanel")
);
...
const AudioPage: React.FC<IProps> = PatchComponent("AudioPage", (props) => {
  ...
  const [generateScreenshot] = useAudioGenerateScreenshot();
      ...
      <AudioMergeModal
        ...
        onClose={(mergedId) => {
          ...
          if (mergedId !== undefined && mergedId !== audio.id) {
            // By default, the merge destination is the current audio, but
  ...
  function maybeRenderAudioGenerateDialog() {
    if (isGenerateDialogOpen) {
      return (
        <GenerateDialog
          selectedIds={[audio.id]}
          onClose={() => {
    ...
    >
      <div>
        <Nav variant="tabs" className="mr-auto">
          <AudioPageTabs {...props}>
            ...
            <Nav.Item>
              <Nav.Link eventKey="audio-video-filter-panel">
      ...
      <Tab.Content>
        <AudioPageTabContent {...props}>
          ...
          <Tab.Pane eventKey="audio-video-filter-panel">
            <AudioVideoFilterPanel audio={audio} />
          </Tab.Pane>
          <Tab.Pane
  ...
  return (
    <>
      ...
      {maybeRenderAudioGenerateDialog()}
...
const AudioLoader: React.FC<RouteComponentProps<IAudioParams>> = ({
  ...
  const currentQueueIndex = useMemo(
    () => queueAudios.findIndex((s) => s.id === id),
    [queueAudios, id]
  );
  ...
  function getAudioPage(audioID: string) {
    ...
    const index = queueAudios.findIndex((s) => s.id === audioID);
