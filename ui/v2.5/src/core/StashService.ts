...
export const queryFindFullAudiosByID = (audioIDs: number[]) =>
...
export const queryAudioByPathRegex = (filter: GQL.FindFilterType) =>
  client.query<GQL.FindAudiosByPathRegexQuery>({
    query: GQL.FindAudiosByPathRegexDocument,
    variables: { filter },
  });
...
const audioMutationImpactedTypeFields = {
  ...
  Studio: ["audio_count", "performer_count"],
  Tag: ["audio_count"],
};
...
export const mutateCreateAudio = (input: GQL.AudioCreateInput) =>
  client.mutate<GQL.AudioCreateMutation>({
    ...
    update(cache, result) {
      ...
      // if we're reassigning files, refetch files from other audios
      if (input.file_ids?.length) {
        const obj = { __typename: "Audio", id: audio.id };
        evictTypeFields(
          cache,
          {
            ...audioMutationImpactedTypeFields,
            Audio: ["files"],
          },
          cache.identify(obj) // don't evict this audio
        );
      } else {
...
export const useAudioIncrementO = (id: string) =>
  GQL.useAudioAddOMutation({
    ...
    update(cache, result, { variables }) {
      ...
      const audio = cache.readFragment<GQL.SlimAudioDataFragment>({
        id: cache.identify({ __typename: "Audio", id }),
        fragment: GQL.SlimAudioDataFragmentDoc,
        fragmentName: "SlimAudioData",
      });

      if (audio) {
        // if we have the audio, update performer o_counters manually
        for (const performer of audio.performers) {
          cache.modify({
            id: cache.identify(performer),
            fields: {
              o_counter(value) {
...
export const useAudioResetO = (id: string) =>
  GQL.useAudioResetOMutation({
    ...
    update(cache, result) {
      ...
      if (audio) {
        // if we have the audio, update performer o_counters manually
        const old_count = audio.o_counter ?? 0;
        for (const performer of audio.performers) {
          cache.modify({
            id: cache.identify(performer),
            fields: {
